 ...
 * @author LM
 */
@WebServlet(name = "EditoraExcluir", urlPatterns = {"/EditoraExcluir"})
public class EditoraExcluir extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if(dao.excluir(idEditora)){
                mensagem = "Editora excluída com Sucesso!";
            }else {
                mensagem = "Problemas ao excluir Editora!";
            }
            request.setAttribute("mensagem", mensagem);
            response.sendRedirect("EditoraListar");
        } catch(Exception ex){
            System.out.println("Problemas no Servlet ao EXCLUIR Editora! Erro: " + ex.getMessage());
            ex.printStackTrace();
        }
    }
