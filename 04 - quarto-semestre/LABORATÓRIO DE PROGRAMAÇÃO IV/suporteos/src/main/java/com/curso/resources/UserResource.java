...
public class UserResource {
    ...
    @GetMapping //exemplo http://localhost:8080/user
    public ResponseEntity<List<UserDTO>> findAll(){
        return ResponseEntity.ok().body(userService.findAll());
    }

    @GetMapping(value = "/{id}") // exemplo http://localhost:8080/user/1
    public ResponseEntity<UserDTO> findById(@PathVariable Long id){
        User obj = this.userService.findbyId(id);
        return ResponseEntity.ok().body(new UserDTO(obj));
    }

    @GetMapping(value = "/cpf/{cpf}") // exemplo http://localhost:8080/user/cpf/1
    public ResponseEntity<UserDTO> findByCpf(@PathVariable String cpf){
        User obj = this.userService.findbyCpf(cpf);
        return ResponseEntity.ok().body(new UserDTO(obj));
    }

    @GetMapping(value = "/email/{email}") // exemplo http://localhost:8080/user/email/1
